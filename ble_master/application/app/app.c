...
///////////// BLE stack Initialization ////////////////
	...
	/*<! THang nay khoi tao nen tang MCU cho ble chay duoc, vi du timer de timing trong ble, ... */
	blc_ll_initBasicMCU();
	/*<! Vao state standby trong ble, kieu nhu nap tham so vao trong de init driver */
	blc_ll_initStandby_module(mac_public);				//mandatory
	/*<! Init scanner */
	blc_ll_initScanning_module(mac_public); 			//scan module: 		 mandatory for BLE master,
	blc_ll_initInitiating_module();						//initiate module: 	 mandatory for BLE master,
	blc_ll_initConnection_module();						//connection module  mandatory for BLE slave/master
	/*<! Role la master nhung single conn, neu muon thu multi tu di vao sdk ma tim */
...
int main_idle_loop (void)
{
	...
	//bdang, gui req exchange mtu, flag se duoc set sau khi establish connection duoc goi
    if(mtu_req_pending &&
       IS_CONNECTION_HANDLE_VALID(cur_conn_device.conn_handle) &&
       blc_ll_getCurrentState() == BLS_LINK_STATE_CONN)
    {
        mtu_req_pending = 0;
        blc_att_requestMtuSizeExchange(cur_conn_device.conn_handle, BDANG_MTU_EXCHANGE);
    }
