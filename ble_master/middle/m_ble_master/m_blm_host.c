...
u16 final_MTU_size = 23;	//default mac dinh cua sdk

//bdang, flag them vao
u8 mtu_req_pending = 0;
static u8 sdp_need_after_mtu = 0; //flag nay su dung de danh sau discovery sau khi mtu exchange xong
 ...
 */
 ...
 //bdang, mac cua thiet bi can ket noi
static const u8 target_mac[6] = {
    0x96, 0x19, 0x01, 0x13, 0x00, 0x00   // 00:00:13:01:19:96
};

//bdang, func xu li adv packet
int blm_le_adv_report_event_handle(u8 *p)
{
	...
	{
		if (memcmp(pa->mac, target_mac, 6) == 0) {
			tlkapi_printf(UART_PRINT_DEBUG_ENABLE,
			"mac target found: %02X:%02X:%02X:%02X:%02X:%02X\r\n",
			(unsigned)pa->mac[5], (unsigned)pa->mac[4], (unsigned)pa->mac[3],
			(unsigned)pa->mac[2], (unsigned)pa->mac[1], (unsigned)pa->mac[0]);
		}
		else {
			return 0;
		}
...
int blm_le_connection_establish_event_handle(u8 *p)
{
	...
	if (pConnEvt->status == BLE_SUCCESS)	// status OK
	{
		...
		//bdang, set flag de ghi nho discovery sau khi mtu exchange
		#if (ACL_CENTRAL_SIMPLE_SDP_ENABLE)
			//new slave device, should do service discovery again
			if (pConnEvt->peerAddrType != serviceDiscovery_adr_type || memcmp(pConnEvt->peerAddr, serviceDiscovery_address, 6)){
				sdp_need_after_mtu = 1;
			}
		#endif
	}
	//set flag de thuc hien exchange trong mainloop
	mtu_req_pending = 1;  
 ...
 * @return     0
 */
//func callback xu ly event trong l2cap, blc_gap_registerHostEventHandler (app_host_event_callback); dong 177 ben app.c
int app_l2cap_handler (u16 conn_handle, u8 *raw_pkt)
{
	...
	if(ptrL2cap->chanId == L2CAP_CID_ATTR_PROTOCOL)  //att data
	{
		...
		//bdang, day la noi xu ly ca request va response tu target
		//Doi voi opcode MTU_REQ tuc la thiet bi target gui reqeust sang minh, thi se dap lai voi blc_att_responseMtuSizeExchange
		//Nguoc lai neu nhu la MTU_RSP tuc la thiet bi chung ta da gui REQ truoc do roi va target phan hoi RSP_MTU thi chi can set MTU nhu binh thuong la duoc
		if(pAtt->opcode == ATT_OP_EXCHANGE_MTU_REQ || pAtt->opcode == ATT_OP_EXCHANGE_MTU_RSP)
		{
			...
			//Khi nhan duoc req exchange mtu tu target
			if(pAtt->opcode ==  ATT_OP_EXCHANGE_MTU_REQ){
				blc_att_responseMtuSizeExchange(conn_handle, BDANG_MTU_EXCHANGE);
			}

			//xu li chung cho ca req va rsp, du gi ket qua cuoi cung la set mtu
			u16 peer_mtu_size = (pMtu->mtu[0] | pMtu->mtu[1]<<8);
			final_MTU_size = min(BDANG_MTU_EXCHANGE, peer_mtu_size);
			blc_att_setEffectiveMtuSize(conn_handle , final_MTU_size);  //important

			#if (ACL_CENTRAL_SIMPLE_SDP_ENABLE)
				if(sdp_need_after_mtu){
					sdp_need_after_mtu = 0;
					app_register_service(&app_service_discovery);
					central_sdp_pending = 1;
				}
			#endif

			tlkapi_printf(UART_PRINT_DEBUG_ENABLE, "MTU set to %d\r\n", final_MTU_size);
		}
		...
		{
			tlkapi_printf(UART_PRINT_DEBUG_ENABLE, "noti received\r\n");
		}
