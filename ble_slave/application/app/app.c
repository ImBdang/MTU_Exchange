...
#define 	ADV_IDLE_ENTER_DEEP_TIME			2 
#define 	CONN_IDLE_ENTER_DEEP_TIME			60  
#define 	USER_IDLE_TIME_IN_ADV				2
...
// 	 13,  DT_COMPLETE_LOCAL_NAME, 				'B', 'd', 'a', 'n', 'g', '_', 't', 'e', 'l', 'i', 'n', 'k',
// 	 2,	 DT_FLAGS, 								0x05, 					// BLE limited discoverable mode and BR/EDR not supported
// 	 4, DT_MANUFACTURER_SPECIFIC_DATA, 			FACTORY_TAG, FACTORY_ID_L, FACTORY_ID_M
// 	//  3,  DT_APPEARANCE, 						0x00, 0x00, 			// 384, Generic Remote Control, Generic category
// 	//  5,  DT_INCOMPLETE_LIST_16BIT_SERVICE_UUID,	0x12, 0x18, 0x0F, 0x18,	// incomplete list of service class UUIDs (0x1812, 0x180F)
// };

extern u8* gen_str_led_state(void);
...
void update_scanrsp_led_state(void)
{
    u8 *msg = gen_str_led_state();   

    static u8 tbl_scanRsp[9];
    tbl_scanRsp[0] = 8; 
    tbl_scanRsp[1] = DT_MANUFACTURER_SPECIFIC_DATA;
    tbl_scanRsp[2] = VALUE_TAG;     
    memcpy(&tbl_scanRsp[3], msg, 6); 

    bls_ll_setScanRspData(tbl_scanRsp, sizeof(tbl_scanRsp));
}

void update_adv_led_state(void)
{
    u8 *msg = gen_str_led_state();   

    static u8 adv_buf[31];
    u8 idx = 0;


    adv_buf[idx++] = 13;                      
    adv_buf[idx++] = DT_COMPLETE_LOCAL_NAME;
    memcpy(&adv_buf[idx], "Bdang_telink", 12);
    idx += 12;


    adv_buf[idx++] = 2;                      
    adv_buf[idx++] = DT_FLAGS;
    adv_buf[idx++] = 0x05;


    adv_buf[idx++] = 11;
    adv_buf[idx++] = DT_MANUFACTURER_SPECIFIC_DATA;
    adv_buf[idx++] = FACTORY_TAG;
    adv_buf[idx++] = FACTORY_ID_L;
    adv_buf[idx++] = FACTORY_ID_M;
    adv_buf[idx++] = VALUE_TAG;
    memcpy(&adv_buf[idx], msg, 6);
    idx += 6;

    //bls_ll_setAdvEnable(BLC_ADV_DISABLE);
    bls_ll_setAdvData(adv_buf, idx);
...
{
	(void)e;(void)p;(void)n;
	if (BLE_WHITELIST_ENABLE)
	{
		bls_ll_setAdvParam( MY_ADV_INTERVAL_MIN, MY_ADV_INTERVAL_MAX,
							ADV_TYPE_CONNECTABLE_UNDIRECTED, app_own_address_type,
							0,  NULL,
							MY_APP_ADV_CHANNEL, ADV_FP_ALLOW_SCAN_ANY_ALLOW_CONN_WL);
							// ADV_FP_NONE);
	}
	else
	{
...
extern u8 otaDataCCC[2];

static void ota_fw_version_req_cb(void)
{
    tlkapi_printf(UART_PRINT_DEBUG_ENABLE, "[OTA] CMD_OTA_VERSION, FW=%s\r\n", FW_VERSION);

    if (blc_ll_getCurrentState() != BLS_LINK_STATE_CONN) return;
    if (!(otaDataCCC[0] & 0x01)) return; 

    bls_att_pushNotifyData(OTA_CMD_OUT_DP_H, (u8*)FW_VERSION, (u16)(sizeof(FW_VERSION) - 1));
}
	...
	if(blc_ll_getCurrentState() == BLS_LINK_STATE_IDLE){ //PM module can not manage Idle state low power.
			...
			if(sendTerminate_before_enterDeep == 2){  //Terminate OK
				analog_write(USED_DEEP_ANA_REG, analog_read(USED_DEEP_ANA_REG) | CONN_DEEP_FLG);
					...
					if (gpio_read(TOUCHPAD_1) && gpio_read(TOUCHPAD_2) && gpio_read(TOUCHPAD_3)) 
					{
						tlkapi_printf(UART_PRINT_DEBUG_ENABLE, "DEEPSLEEP 1 NE\r\n");
		...
		//do not care about keyScan/button_detect power here, if you care about this, please refer to "ble_remote" demo
			...
			if(!ota_is_working && !blc_ll_isControllerEventPending()){  //no controller event pending
				...
				if( blc_ll_getCurrentState() == BLS_LINK_STATE_ADV && !sendTerminate_before_enterDeep && \
					...
					clock_time_exceed(latest_user_event_tick, USER_IDLE_TIME_IN_ADV * 1000000)){

					if (gpio_read(TOUCHPAD_1) && gpio_read(TOUCHPAD_2) && gpio_read(TOUCHPAD_3)) 
					{
						tlkapi_printf(UART_PRINT_DEBUG_ENABLE, "DEEPSLEEP NE\r\n");
						cpu_sleep_wakeup(DEEPSLEEP_MODE_RET_SRAM_LOW16K, PM_WAKEUP_PAD, 0);  //deepsleep
					}
				}
...
//////////////////////////// BLE stack Initialization  Begin //////////////////////////////////
	...
	if (BLE_WHITELIST_ENABLE)
	{
		static const u8 mac_msb[6] = {0xAE, 0x31, 0x87, 0xFB, 0x3D, 0xB8};
		static const u8 mac_lsb[6] = {0xB8, 0x3D, 0xFB, 0x87, 0x31, 0xAE};

		static const u8 phone_mac_msb[6] = {0x6C, 0x48, 0x3F, 0xD1, 0xF2, 0xE6};
		static const u8 phone_mac_lsb[6] = {0xE6, 0xF2, 0xD1, 0x3F, 0x48, 0x6C};
		blc_ll_clearWhiteList();

		blc_ll_addDeviceToWhiteList(0x00, mac_msb);
		blc_ll_addDeviceToWhiteList(0x00, mac_lsb);
		blc_ll_addDeviceToWhiteList(0x00, phone_mac_msb);
		blc_ll_addDeviceToWhiteList(0x00, phone_mac_lsb);

		blc_ll_addDeviceToWhiteList(0x01, mac_msb);
		blc_ll_addDeviceToWhiteList(0x01, mac_lsb);
		blc_ll_addDeviceToWhiteList(0x01, phone_mac_msb);
		blc_ll_addDeviceToWhiteList(0x01, phone_mac_lsb);
	}
	...
	//bdang, set lai MTU cho Rx size cua server
	//sau khi nhan MTU_REQ cua master, slave se kiem tra xem Rx size co du cho master hay khong
	blc_att_setRxMtuSize(BDANG_EXCHANGE_MTU); //set MTU size, default MTU is 23 if not call this API
	...
	//////////// Service Initialization  Begin /////////////////////////
		...
		blc_ota_setOtaProcessTimeout(120);   //OTA process timeout:  30 seconds
		...
		blc_ota_registerOtaFirmwareVersionReqCb(ota_fw_version_req_cb);
	#endif
...
//////////////////////////// User Configuration for BLE application ////////////////////////////
	...
	///////////////////// Power Management initialization///////////////////
		...
		cpu_set_gpio_wakeup (TOUCHPAD_1, Level_Low,1);  //button pin pad low wakeUp suspend/deepSleep
		cpu_set_gpio_wakeup (TOUCHPAD_2, Level_Low,1); 
		cpu_set_gpio_wakeup (TOUCHPAD_3, Level_Low,1); 
...
_attribute_ram_code_ void user_init_deepRetn(void)
{
	...
	advertise_begin_tick = clock_time();
    latest_user_event_tick = advertise_begin_tick;
	...
	bls_ll_setAdvEnable(BLC_ADV_ENABLE);
		...
		cpu_set_gpio_wakeup(TOUCHPAD_1, Level_Low, 1);
		cpu_set_gpio_wakeup(TOUCHPAD_2, Level_Low, 1);
		cpu_set_gpio_wakeup(TOUCHPAD_3, Level_Low, 1);
