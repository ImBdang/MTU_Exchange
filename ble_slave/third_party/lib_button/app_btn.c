...
void app_hold_plus_time(uint32_t plus_value)
{
    app_btn_on_hold_time_fire_event_ms += plus_value;
}

void app_hold_reset_time(void)
{
    app_btn_on_hold_time_fire_event_ms = APP_BTN_ON_HOLD_TIME_FIRE_EVENT_MS;
}
...
void app_btn_initialize(app_btn_config_t *conf)
{
    ...
    if (NULL == conf)
    {
        // blog_error("ERR: cfg NULL ");
        return;
    }

    if (APP_BTN_MAX_BTN_SUPPORT < conf->btn_count)
    {
        // blog_error("ERR: btn support: %d ", conf->btn_count);
        return;
    }
...
void app_btn_scan(void *params)
{
    ...
    if (!m_hw_params)
    {
        // blog_error("ERR: hw params NULL ");
        return;
    }

    if (APP_BTN_MAX_BTN_SUPPORT < m_btn_config.btn_count)
    {
        // blog_error("ERR: btn support: %d ", m_btn_config.btn_count);
        return;
    }
    ...
    for (i = 0; i < m_btn_config.btn_count; i++)
    {
        ...
        if ((m_btn_press_evt >> i) & 0x01)
        {
            ...
            {
                tlkapi_printf(1, "ERROR TOUCHPAD");
            }
            m_btn_press_evt &= ~(1 << i);
        }
        if ((m_btn_release_evt >> i) & 0x01)
        {
            ...
            {
                tlkapi_printf(1, "ERR: cb for release NULL ");
            }
            m_btn_release_evt &= ~(1 << i);
